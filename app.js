...
async function loadArticles(){try{const {data,error}=await db('articles',{select:'*',eq:{published:true},order:{column:'created_at',ascending:false}});if(error)throw error;const grid=document.getElementById('articleGrid');if(!grid)return;if(!data?.length){grid.innerHTML='<p class="empty-state">No articles yet.</p>';return}grid.innerHTML=data.map(a=>`<article class="article-card"><span class="tag">${escapeHtml(a.category||'ARTICLE')}</span><h3>${escapeHtml(a.title)}</h3><p>${escapeHtml(a.content).slice(0,220)}</p><a class="article-read" href="article.html?id=${encodeURIComponent(a.id)}" target="_blank" rel="noopener">Read article →</a><div class="article-byline">Published by ${escapeHtml(a.author||'Design & Create')}</div></article>`).join('')}catch(e){console.warn('Articles load failed:',e)}}
